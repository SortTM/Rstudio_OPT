...
dataDT <<- read.csv("/Users/mohammadhosseinnazari/Downloads/Heart_disease_cleveland_new.csv")
...
classfeatureFitness <- function(string,xx,yy) {
  ...
  acc=mean(dataDT$target == t_pred)
  #Maximise accuracy
  return(-acc)
  
  #Maximise accuracy and minimise the number of features
  #return( mean(dataDT$Risk == t_pred) * (1 - sum(string == 1)/length(string) ) )
}
...
runPSO <-function(){
  ...
  for (i in 1:noRuns){
    ...
    pso.result <- psoptim(par = rep(0, ncol(xx)),lower = lower.bounds, upper = upper.bounds, fn = classfeatureFitness, 
...
parse_pso_result<-function(d, noRuns, noIter){
  data<-abs(d)
  pdata <- matrix(nrow = noIter, ncol = 3)
  for (i in 1:noIter){
    pdata[i,1] = i
    pdata[i,2] = mean(data[i,])
    pdata[i,3] = 1.96*sd((data[i,]))/sqrt(noRuns)   #compute the length of error bar. 
  }
